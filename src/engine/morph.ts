...
export const drawMorph: ModeDraw = (ctx, size, t, dark, o) => {
  ...
  const tc = safeT % (SEG * K);
  const rawK = Math.floor(tc / SEG);
  const k = Number.isFinite(rawK) ? Math.min(K - 1, Math.max(0, rawK)) : 0;
  ...
  const pA = CYCLE[k] || CIRCLE;
  const pB = CYCLE[(k + 1) % K] || CIRCLE;
  const M = 160;
